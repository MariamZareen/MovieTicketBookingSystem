...
export default function Dummy() {
 ...
 const { enqueueSnackbar } = useSnackbar();
  ...
  useEffect(()=>{
     axios.get(`https://demo-omega-ochre.vercel.app0/dummy/${id}?movieName=${movieName}?date=${date}&day=${day}&theatreName=${theatreName}&theatreLocation=${theatreLocation}&slot=${slot}&noOfSeats=${noOfSeats}&price=${price}`)
     .then((res)=>{
        console.log(res);
     })
     .catch(()=>{
    ...
    <div className='w-screen h-screen bg-slate-900 flex justify-center items-center'>
      <div className='flex justify-center items-center flex-col w-[50%] h-[85%] bg-slate-300 rounded p-7 text-2xl'>
          ...
          <div className="rightdiv w-[50%] m-2 p-5 h-[98%] flex flex-col justify-around">
           ...
           <center>
           <button onClick={makePayment} className='rounded bg-[#f09631] text-slate-300 text-2xl p-3 w-[250px] mt-6'>Book</button>
             </center>         
          </div>
      </div>
    </div>
